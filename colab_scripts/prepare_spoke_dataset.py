#!/usr/bin/env python3
"""Build the rotating-frame average density and estimate spoke rotation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", default="outputs/rotating_average", help="Output directory.")
...
def save_phase_plot(out_dir: Path, times: np.ndarray, phases: np.ndarray, fit: np.ndarray, omega: float) -> None:
    ...
    ax[0].set_title(f"Spoke rotation fit: omega = {omega:.12g}")
...
def save_average_plot(out_dir: Path, mean_pattern: np.ndarray, std_pattern: np.ndarray, last_frame: np.ndarray, mask: np.ndarray) -> None:
    ...
    im0 = ax[0].imshow(np.where(mask, last_frame, np.nan), origin="lower", cmap="magma")
    ax[0].set_title("last stationary frame")
    fig.colorbar(im0, ax=ax[0], fraction=0.046)

    im1 = ax[1].imshow(mean_pattern, origin="lower", cmap="magma")
    ax[1].set_title("mean rotating-frame density")
...
def main() -> None:
    ...
    print(f"Saved rotating average to {out_dir / 'rotating_average.npz'}")
